...
parser.add_argument("-s", "--son",type = int,required = True, help="col of propositus")
parser.add_argument("-f", "--father",type = int,required = True, help="col of father")
parser.add_argument("-m", "--mom",type = int,required = True, help="col of mother")
...
if __name__ == "__main__":
    with open(args.whanno_txt,"r",encoding='utf-16') as txt:
        fileName = os.path.basename(args.whanno_txt)
        out_file = os.path.splitext(fileName)[0] + '_mutation_compound_heterozygous_discover.txt'
        after_mutation_list = []
        first = txt.readline().strip()
        # print(first)
        for line1 in txt:
            line = line1.strip()
            snp = line.split("\t")
        ...
        result_part1 = compound_heterozygous_discover(after_mutation_list)

    with open(args.intron_txt,"r",encoding='utf-16') as txt2:
        for intron in txt2:
            new_intron = intron.strip()
            result.append(new_intron)
